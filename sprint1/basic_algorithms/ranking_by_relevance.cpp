    ...
    const set<string>& query_words)
{
    ...
    vector<pair<int, int>> found_docs;

    for (const auto& document: documents){
        const int relevance = MatchDocument(document, query_words);
        if (relevance > 0){
            found_docs.push_back({relevance, document.first});
        }
    }
    return found_docs;
}
...
vector<pair<int, int>> FindTopDocuments(const vector<pair<int, vector<string>>>& documents,
    ...
    const set<string> parsed_query = ParseQuery(raw_query, stop_words);
    auto found_documents = FindAllDocuments(documents, parsed_query);
    ...
    for (auto& doc: found_documents){
        swap(doc.first, doc.second);
    }
